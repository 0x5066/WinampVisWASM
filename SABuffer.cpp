...
static IppStatus ippsSet_32f(float val, float* pDst, int len) {
    if (!pDst || len <= 0) return ippStsNullPtrErr;
...
static IppStatus ippsMul_32f_I(const float* pSrc, float* pSrcDst, int len) {
    if (!pSrc || !pSrcDst || len <= 0) return ippStsNullPtrErr;
...
static IppStatus ippsWinHann_32f_I(float* pSrcDst, int len) {
    if (!pSrcDst || len <= 0) return ippStsNullPtrErr;

    const float PI = 3.14159265358979323846f;
    for (int i = 0; i < len; i++) {
        pSrcDst[i] *= 0.5f * (1.0f - cosf((2.0f * PI * i) / (len - 1)));
    }
    return ippStsNoErr;
}

static void nsutil_window_FillHann_F32_IP(float *window, size_t number_of_samples) {
    ippsSet_32f(1.0f, window, number_of_samples);
    ippsWinHann_32f_I(window, number_of_samples);
}

static void nsutil_window_Multiply_F32_IP(float *signal, const float *window, size_t number_of_samples) {
