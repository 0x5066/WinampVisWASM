...
int sa_curmode; // Initial value
int _srate = 44100; // Default sample rate

extern "C" {
    // Getter for config_sa
    unsigned char get_sa_curmode() {
        return sa_curmode;
    }

    // Setter for config_sa
    void set_sa_curmode(unsigned char value) {
        sa_curmode = value;
    }
}
...
void makeOscData(char *tempdata, char *data_buf, int little_block, int channels, int bits)
{
	...
	for (x = 0; x < 75; x ++)
	{
		...
		tempdata[x] = (char)std::lrint(val / (float)channels);  // average the channels
		//std::cout << (int)tempdata[x] << std::endl;
	}
}
...
/* 	if (config_replaygain)
	...
	for (int x = 0; x < 75; x ++)
	{
		...
		float thisValue = 0;
		int bin = std::lrint(floor(binF));
		int end = std::lrint(floor(next));
		end = std::min(end, 255);
		...
		tempdata[x]=std::lrint(fastmin(thisValue, 255.f));
	}
...
void sa_addpcmdata(void *_data_buf, int numChannels, int numBits, int ts)
{
	...
	case 4:
		...
		return;
	case 2:
		makeOscData(tempdata,data_buf,BUFFER_SIZE,numChannels, numBits); // 576
		sa_add(tempdata,ts,2);
		return ;
	case 3:
		makeOscData(tempdata+75,data_buf,BUFFER_SIZE,numChannels, numBits); // 576
		// fall through!
	case 1:
	...
	bool done=false;
	size_t samples=BUFFER_SIZE; // 576
	while (samples)
	{
